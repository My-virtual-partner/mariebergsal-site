 ...
 */
function render_fields_for_new_projects($fields_array, $project_type_id) {
...
print_r($steps); */
	...
	/* echo"<pre>";
	print_r($json_data_as_array);die; */
    $order = wc_get_order($_GET["order-id"]);
	
/* if(in_array($current_user_role,$project_role)){ */
    if ($step === null) {
        ...
        foreach ($fields_array as $key => $value) {
            ...
            foreach ($fields_array[$key] as $field) {
                ...
                foreach ($field as $single_field) {
					  if(!in_array($current_user_role,$single_field["role"]))
						break;

                    if ($single_field["acf_fc_layout"] == "textbox") {

                        render_textboxs($single_field, $json_data_as_array['imm-sale-value_' . create_id_from_name($single_field["name"])]);

                    } elseif ($single_field["acf_fc_layout"] == "text_area") {
						
                        render_text_areas($single_field, $json_data_as_array['imm-sale-value_' . create_id_from_name($single_field["name"])]);

                    } elseif ($single_field["acf_fc_layout"] == "dropdown") {

                        render_dropdowns($single_field, $json_data_as_array['imm-sale-value_' . create_id_from_name($single_field["name"])]);

                    } elseif ($single_field["acf_fc_layout"] == "file") {

                        render_file_inputs($single_field, $json_data_as_array['imm-sale-value_' . create_id_from_name($single_field["name"])]);

                    } elseif ($single_field["acf_fc_layout"] == "webshop") {

                        render_webshops($single_field["webshop_show_category_dropdown"], $single_field["webshop_category"], $single_field["required"]);
        ...
        $username = getCustomerName($userid->ID);
 ...
 Denna redigeras just nu av ' . $username . '. Glöm inte att trycka på "spara" i steget "PI".
        ...
        $html_completed_dropdown .= "<option " . $selected_d . " value='3' >" . __("Välj") . "</option>";
        ...
        foreach ($fields_array[$step] as $field) {
            ...
            foreach ($field as $single_field) {
                ...
                if ($single_field["acf_fc_layout"] == "textbox") {
                    ...
                    render_dates($single_field, $json_data_as_array['imm-sale-value_' . create_id_from_name($single_field["name"])]);

                } elseif ($single_field["acf_fc_layout"] == "time") {

                    render_times($single_field, $json_data_as_array['imm-sale-value_' . create_id_from_name($single_field["name"])]);
                   ...
                   render_text_areas($single_field, $json_data_as_array['imm-sale-value_' . create_id_from_name($single_field["name"])]);
                    ...
                    render_file_uploads($_GET['order-id']);
...
function render_file_uploads($order_id) {
   $errorCode = get_post_meta($order_id,'new_error',true);
if(count($errorCode) !='0'){
	foreach($errorCode as $err){
		echo '<p class="bg-danger">'.$err.'</p><br>';	
	}

		delete_post_meta( $order_id, 'new_error', $errorCode);
}
    ?>


    <div class="top-buffer-half col-lg-12" id="all_files_project" >
        <label for="files_type"><strong><?php echo __("Ladda upp dokument"); ?></strong></label>
        <div class = "upload-arbet-form-order-files">
            ...
            <div class = "form-group">
                <input value="Ladda upp" type="submit" name="forward_step" class="btn btn-brand spara_och_fortsatt btn  btn-alpha btn-show-overlay btn-block" id="">
            </div>
        </div>
...
function render_dates($args, $current_value = null) {
    ...
    $html .= "<label class='top-buffer-half' for='" . create_id_from_names($args['name']) . "'> " . $args['name'] . $required_span . "</label>";
    $html .= "<input value='" . $args['name'] . "' type='hidden' name='imm-sale-label_" . create_id_from_names($args['name']) . "'>";
    $html .= "<input  title='test tooltip' " . $required_setting . " value='" . $current_value["value"] . "' type='date' name='imm-sale-value_" . create_id_from_names($args['name']) . "' class='form-control $required_setting' id='" . create_id_from_names($args['name']) . "'>";
    $html .= "</div>";

    echo $html;
}

function render_times($args, $current_value = null) {
    $required = $args["required"];
    $required_setting = null;
    $required_span = null;
    ...
    $html .= "<label class='top-buffer-half' for='" . create_id_from_names($args['name']) . "'> " . $args['name'] . $required_span . "</label>";
    $html .= "<input value='" . $args['name'] . "' type='hidden' name='imm-sale-label_" . create_id_from_names($args['name']) . "'>";
    $html .= "<input  title='test tooltip' " . $required_setting . " value='" . $current_value["value"] . "' type='time' name='imm-sale-value_" . create_id_from_names($args['name']) . "' class='form-control $required_setting' id='" . create_id_from_names($args['name']) . "'>";
    $html .= "</div>";

    echo $html;
}

function render_textboxs($args, $current_value = null) {
    $required = $args["required"];
    $required_setting = null;
    $required_span = null;
    ...
    $html .= "<label class='top-buffer-half' for='" . create_id_from_names($args['name']) . "'> " . $args['name'] . $required_span . "</label>";
    $html .= "<input value='" . $args['name'] . "' type='hidden' name='imm-sale-label_" . create_id_from_names($args['name']) . "'>";
    $html .= "<input  title='test tooltip' " . $required_setting . " value='" . $current_value["value"] . "' type='text' name='imm-sale-value_" . create_id_from_names($args['name']) . "' class='form-control $required_setting' id='" . create_id_from_names($args['name']) . "'>";
    $html .= "</div>";

    echo $html;
}

function render_text_areas($args, $current_value = null) {
    $keys = array('Information-frn-kund', 'Arbetsorder');
    $required = $args["required"];
    $required_setting = null;
    $required_span = null;

    if (in_array(create_id_from_names($args['name']), $keys)) {

        if (get_post_meta($_GET["order-id"], create_id_from_names($args['name']), true)) {
            $newvalue = get_post_meta($_GET["order-id"], create_id_from_names($args['name']), true);
        } else {
    ...
    $html .= " <textarea " . $required_setting . "  class='form-control' rows='5' name='imm-sale-value_" . create_id_from_name($args['name']) . "'
                                  id='" . create_id_from_names($args['name']) . "'>" . $newvalue . "</textarea>";
    $html .= "</div>";

    echo $html;
}

function render_file_inputs($args, $current_value = null) {
    $html = "<div class='col-lg-12'>";

    $html .= "<label class='top-buffer-half' for='" . create_id_from_names($args['name']) . "'> " . $args['name'] . "</label><br>";
    if ($current_value["value"]) {
        $html .= '<div id="' . create_id_from_names($args['name']) . '">';
        $html .= "<a target='blank' href='" . $current_value["value"] . "'><img src='" . $current_value["value"] . "' height=\"80\" width=\"80\"></a>";
        $html .= "<a target='blank' href='#' data_placement='" . $current_value['summary_placement'] . "' data_order='" . $_GET['order-id'] . "' data_name = '" . 'imm-sale-value_' . create_id_from_names($args['name']) . "' data_div_id='" . create_id_from_names($args['name']) . "' class='tabort_bild_render'>Ta bort</a>";
        $html .= '</div>';
    }
    $html .= "<input value='get_image' type='hidden' id='image_file' name='image_file'>";
    $html .= "<input value='" . $args['placement_summary'] . "' type='hidden' name='imm-sale-summary-placement_" . create_id_from_names($args['name']) . "'>";
    $html .= "<input value='" . $args['name'] . "' type='hidden' name='imm-sale-label_" . create_id_from_names($args['name']) . "'>";
    $html .= "<input value='" . $current_value["value"] . "' type='hidden' name='imm-sale-file_" . create_id_from_name($args['name']) . "'>";
    $html .= "<input value='" . $current_value["image_description"] . "' type='text' name='imm-sale-image-description_" . create_id_from_names($args['name']) . "' class='form-control' id='" . create_id_from_names($args['name']) . "' placeholder= " . __('Bildbeskrivning') . ">";
    $html .= "<input value='" . $current_value["value"] . "' accept='image/x-png,image/gif,image/jpeg'  type='file' name='imm-sale-value_" . create_id_from_names($args['name']) . "' class='' id='" . create_id_from_name($args['name']) . "'>";
    $html .= "</div>";

    echo $html;
}

function render_type_services() {
    include('fields/type_service.php');
}

function render_dropdowns($args, $current_value = null) {
...
$name = $args['name'];
    ...
    $html = "<div class='col-lg-6'>";
    $html .= "<label class='top-buffer-half' for='" . create_id_from_names($name) . "'> " . $name . $required_span . "</label>";
    $html .= "<input value='" . $name . "' type='hidden' name='imm-sale-label_" . create_id_from_names($name) . "'>";

    $html .= "<select class='form-control js-sortable-select' id='" . create_id_from_names($name) . "' name='imm-sale-value_" . create_id_from_names($name) . "'>";
...
function render_webshops($show_dropdown = null, $product_categories = null, $required_option = null) {
...
function create_id_from_names($name) {
